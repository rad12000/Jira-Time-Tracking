class AppStorage {
  ...
  /**
   * @typedef {"basic-auth" | "bearer-token"} AuthMode

  /**
   * @param {AuthMode} mode
   */
  static async setActiveAuthMode(mode) {
    await this.#setItemAsync("activeAuthMode", mode);
  }

  /**
   * @returns {Promise<AuthMode>}
   */
  static async getActiveAuthMode() {
    const mode = await this.#getItemAsync("activeAuthMode");
    return mode ?? "basic";
  }

  static async getAuth() {
    const mode = await this.getActiveAuthMode();
    switch (mode) {
      case "basic-auth":
        return await getAtlassianAuthAsync();
      case "bearer-token":
        return await getJiraBearerToken();
      default:
        return "";
    }
  }
  ...
  static setJiraBearerToken = async (authStr) => {
    return await this.#setItemAsync("bearerToken", authStr);
  };

  static getJiraBearerToken = async () => {
    const val = await this.#getItemAsync("bearerToken");
