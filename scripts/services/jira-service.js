...
class JiraService {
  ...
  async refreshClientAsync() {
    const auth = AppStorage.getActiveAuthMode();
  ...
  async #getProjectStatusTypesAsync(projectKey) {
    ...
    try {
      return (await this.#httpClient.getAsync(`project/${projectKey}/statuses`))
        .issueTypes;
    } catch (e) {
      return [];
    }
  ...
  async #getProjectIssueTypesAsync(projectKey) {
    ...
    try {
      return (await this.#httpClient.getAsync(`project/${projectKey}`))
        .issueTypes;
    } catch (e) {
      return [];
    }
  ...
  async getIssueSuggestionsAsync(str) {
    ...
    const res = await this.#httpClient.getAsync(
      "issue/picker",
      queryParams,
      signal,
    );
  ...
  async getPlainIssueSuggestionsAsync(str) {
    ...
    let [history, current] = (
      await this.#httpClient.getAsync("issue/picker", queryParams)
    ).sections;
