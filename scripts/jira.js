...
async function setup() {
  ...
  document
    .getElementById("issue-types")
    .addEventListener("click", displayIssueTypesAsync);
  document
    .getElementById("status-types")
    .addEventListener("click", displayStatusTypesAsync);
  document
    .getElementById("project-search")
    .addEventListener("input", searchProjectsAsync);
  document
    .getElementById("set-base-url")
    .addEventListener("click", updateBaseUrl);
  document
    .getElementById("set-basic-auth")
    .addEventListener("click", setBasicAuth);
  document
    .getElementById("set-bearer-token")
    .addEventListener("click", setBearerToken);
  document
    .getElementById("set-assignee")
    .addEventListener("click", setAssigneeOnly);
  const selector = document.getElementById("auth-selector");
  selector.onchange = async (e) => {
    await AppStorage.setActiveAuthMode(e.target.value);
...
async function syncCurrentAuth() {
  const activeAuthMode = await AppStorage.getActiveAuthMode();
  const selector = document.getElementById("auth-selector");
  selector.value = activeAuthMode;
  document.querySelectorAll(".set-auth").forEach((el) => {
    if (el.id === `set-${activeAuthMode}`) {
...
function updateBaseUrl() {
  ...
  AppStorage.setAtlassianDomainAsync(SearchOptions.domain).then((_) => {
    jiraService.refreshClientAsync();
...
function setBasicAuth() {
  var email = prompt(
    "Please enter the email associated with your Atlassian account",
  );
  var apiToken = prompt("Please provide your Jira api token");

  if (email && apiToken) {
    const basicAuth = `Basic ${btoa(`${email}:${apiToken}`)}`;
    AppStorage.setAtlassianAuthAsync(basicAuth).then((_) =>
      jiraService.refreshClientAsync(),
    );
  } else {
...
function setBearerToken() {
  const token = prompt("Please provide your Jira Personal Access Token");
  if (token) {
    const basicAuth = `Bearer ${token}`;
    AppStorage.setJiraBearerToken(basicAuth).then((_) =>
      jiraService.refreshClientAsync(),
    );
  } else {
    alert("You must provide a valid access token!");
  }
}
...
async function displayStatusTypesAsync() {
  ...
  const types =
    SearchOptions.statusTypes ?? (await jiraService.getStatusTypes());

  let output = "";
  for (const type of types) {
    const enabled =
      SearchOptions.enabledStatuses.indexOf(type.id) > -1 ||
      SearchOptions.enabledStatuses.indexOf(`${type.id}`) > -1;
...
async function searchProjectsAsync(e) {
  ...
  const projectSuggestions =
    document.getElementsByClassName("project-suggestion");
...
export async function showJiraSuggestionsAsync(e) {
  ...
  if (issueCount() < 3) {
    ...
    const altIssues = await jiraService.getIssueSuggestionsAsync(
      searchVal,
      false,
    );
  ...
  if (issueCount() === 0) {
    ...
    const altIssues =
      await jiraService.getPlainIssueSuggestionsAsync(searchVal);
